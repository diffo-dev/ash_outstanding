defmodule ParentChildTest do
  ...
  describe "parent child expectations" do
    test "parent child expectations - outstanding child" do
      ...
      expected = expected_parent |> Ash.Changeset.for_update(:update, add_child: [expected_child.id]) |> Ash.update!()
        |> Ash.load!([:children])
      actual = Generator.generate(Generator.service(name: :actual_parent, state: :active, status: :working))
        |> Ash.load!([:children])
    ...
    test "parent child expectations - outstanding, child status" do
      ...
      actual_child = Generator.generate(Generator.service(name: :actual_child, state: :active, status: :failed))
        |> Ash.load!([:children])
      actual = actual_parent |> Ash.Changeset.for_update(:update, add_child: [actual_child.id]) |> Ash.update!()
      |> Ash.load!([:children])
      assert Outstanding.outstanding?(expected, actual) == true
      outstanding = Outstanding.outstanding(expected, actual)
      refute outstanding.children == nil
    ...
    test "parent child expecations - resolved" do
      ...
      expected = expected_parent |> Ash.Changeset.for_update(:update, add_child: [expected_child.id]) |> Ash.update!()
        |> Ash.load!([:children])
      actual_parent = Generator.generate(Generator.service(name: :actual_parent, state: :active, status: :working))
      actual_child = Generator.generate(Generator.service(name: :actual_child, state: :active, status: :working))
        |> Ash.load!([:children])
      actual = actual_parent |> Ash.Changeset.for_update(:update, add_child: [actual_child.id]) |> Ash.update!()
      |> Ash.load!([:children])
